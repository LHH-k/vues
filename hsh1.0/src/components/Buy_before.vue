  ...
  <div class="buy_360" v-if="userInfo != null">
    ...
    <div class="unHasId" v-else-if="userInfo == ''">
        ...
        <van-cell-group style="margin-bottom:10px" class="icon-field">
          ...
          <van-field :value="form.phone" label="" placeholder="请填写您的手机号码" readonly></van-field>
        </van-cell-group>
    ...
    <div style="text-align:center;margin:20px 0;color:#7e7e7e;" v-if="$route.params.type == 3">
...
<script>
  ...
  export default{
    ...
    created(){
      ...
      //是否是体验卡
      // this.$http.get(`${BASE_URL}/fnw/get/NewUserExpPriority/${localStorage.getItem('phone')}`).then((res)=>{
        if(this.$route.params.type == 3){
          // if(res.data.res){
          //   this.product_id = 28;
          // }else{
    ...
    methods:{
      ...
      goNext(t){
        ...
        if(!this.userInfo){
          ...
          // }
          //验证验证码正确性
          // this.$http.get(baseURL+"/user/get/phoneCodeCheck/" + this.form.phone +"?code=" +this.form.yzm).then(function(res) {
          //   if (res.data.res) {
          ...
          this.$http.post(`${BASE_URL}/fnw/post/NewUserAddress/${this.form.phone}`,adds).then((res)=>{
            if(res.data.res){
              ...
              if(this.$route.params.type == 3){
                ...
                this.$http.post(`${BASE_URL}/fnw/post/NewOrder/${localStorage.getItem('phone')}`,datas).then((res1)=>{
                  if(res.data.res){
                    this.$router.replace({ name: 'pay', params: { type:t} , query:{order_id:res1.data.data}});
                    this.globalToast.clear();
                  }else{
                ...
                //不生成订单，呼叫时再生成
                this.$router.replace({name:'callService',params:{type:t},query:{add_id:res.data.data.id}});
              }
            }
          }).catch((res)=>{})
        }else{
          ...
          this.$http.post(`${BASE_URL}/fnw/post/NewOrder/${localStorage.getItem('phone')}`,datas).then((res)=>{
            if(res.data.res){
              this.globalToast.clear();
              this.$router.replace({ name: 'pay', params: { type:t} , query:{order_id:res.data.data}});
            }else{
              this.$toast.fail('该地址已存在订单，请勿重复购买！如有未支付订单，请到‘我的-未支付订单’中支付！');
            }
          }).catch((res)=>{})
