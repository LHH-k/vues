    ...
    <div class="stit m-10">服务信息</div>
			<div class="box box1">
				<div class="box-t my-field list-field">
					...
					<van-field :value="confirm_info.mobile" label="手机号码：" disabled></van-field>
			...
			<van-cell-group style="margin-top:10px;" class="money">
			...
			<div class="m-10" v-if="confirm_info.cat_id != 1 && confirm_info.cat_id != 2">
				<my-upload :pic-up="picUp" ind='0' txt='维修师傅和服务现场合照' @childget='getImg'></my-upload>
				<p class="prompt">提示：请您上传维修师傅和服务现场合照，以保障后续服务跟踪。</p>
			</div>
    ...
    //从组件获取img数据
		...
		next: function next() {
			...
			if(this.confirm_info.cat_id != 1 && this.confirm_info.cat_id != 2){
				if (!this.imgs.length) {
					this.$toast.fail('请上传维修师傅和服务现场合照！');
					return false;
				}
			}
			//支付方式
			...
			this.$http.get(`http://wx.funlifeday.com/web/wechat/user/get/pay`, {params:datas}).then((res)=>{
					this.globalToast.clear();
					if (!res.data.data.payment) {
						...
						window.location.href = 'http://wx.funlifeday.com/web/wechat/user/enter/payForService/' + res.data.data.id + '?url=' + myUrl;
