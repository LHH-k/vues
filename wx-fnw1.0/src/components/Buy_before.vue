...
<script>
  ...
  export default {
    ...
    created(){
      ...
      //product_id选择
      if(this.$route.params.type == 3){ //家庭开锁
        this.product_id = 2;
      }else if(this.$route.params.type == 4){ //家庭开锁体验版
          this.product_id = 28;
      }else{
        this.product_id = 1; 
      }
    ...
    methods:{
      ...
      getPhoneCode: function getPhoneCode() {
        ...
        console.log(this.isGet,this.form.phone)
        //是否可以发送
        if (this.isGet) {
          if (!regPhone.test(this.form.phone)) {
            ...
            this.isGet = false;
            //获取验证码
            this.$http.get('http://wx.funlifeday.com/web/wechat/user/get/phoneCode/' + this.form.phone).then(function (res) {
              if (res.data.res) {
      ...
      goNext(t){
        ...
        if(!this.userInfo){
          ...
          //验证验证码正确性
          this.$http.get("http://wx.funlifeday.com/web/wechat/user/get/phoneCodeCheck/" + this.form.phone +"?code=" +this.form.yzm).then((res)=>{
            if (res.data.res) {
              //新地址提交
              let adds = {
                name:	this.form.user,	
