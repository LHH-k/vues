    ...
    <div class="box box1">
      ...
      <div class="box-c bg-white my-field">
        <van-cell-group>
          ...
          <van-field v-if="product_id!=39 && product_id!=40" v-model="form.service.name" label="服务项目：" placeholder="请选择您需要服务的项目" @focus="addsShow = true" readonly ></van-field>
          <van-field v-else :value="form.service.name" label="服务项目：" disabled></van-field>
...
export default {
  ...
  created: function created() {
    ...
    if(this.product_id == 39){
      this.form.service = {id:this.product_id,name:'家庭开锁'};
    }else if(this.product_id == 40){
      this.form.service = {id:this.product_id,name:'家庭疏通'};
    }
    // else if(this.product_id == 3){
    //   this.form.service = {id:this.product_id,name:'热水器维修'};
    // }
    ...
    // }else if(this.$route.query.add_id){
      ...
      //   //获取清洗的项目根据product_id来获取
      //   this.$http.get(`${BASE_URL}/fnw/get/CatsByProductId/${localStorage.getItem('phone')}?id=${this.product_id}`).then((res)=>{
      //     if(res.data.res){
      //       // 筛选可选服务类型
      //       this.canServices = res.data.data;
      //     }
