        ...
        <ul>
          <li v-for="(item,index) in myService" :key="index" class="my-order-list">
            ...
            <div class="list-info">
              ...
              <div class="list-list" v-if='item.product_id==31 || item.product_id==30'>
                <div class="l">卡号：</div>
            ...
            <div class="p-20" style="margin: 20px 0;">					
              <van-button v-if="item.product_id==31 || item.product_id==30" class='btn-primary' size="large" @click="callPhone()">电话预约服务</van-button>
              <van-button v-else class='btn-primary' size="large" @click="callS(item.id)">呼叫服务</van-button>
            </div>
          </li>
        </ul>
      </div>
