...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      goNext(product_id){
        if(this.isC){
          // if(product_id!=39 && product_id!=40){
            this.$router.push({name:'buy_before',params:{type:product_id},query:{add_id:this.isC}});
          // }else{
          //   this.$router.push({name:'callService',params:{type:product_id},query:{add_id:this.isC}});
          // }
        }else{
