      ...
      this.$http.get(`${BASE_URL}/fnw/get/UserUnpayOrders/${localStorage.getItem('phone')}?order_id=${this.$route.query.order_id}`).then((res)=>{
        if(res.data.res){
          ...
          if(this.type == 3){
            ...
            this.$router.replace({ name: 'success', params: { type:'callQx'}});
          }else{
    ...
    methods:{
      pay(id){
        if(this.pay_mode){
            ...
            this.$http.post('//wx.funlifeday.com/web/wechat/user/enter/payReadyForNewOrder',{order_id:this.$route.query.order_id, url:location.href, total_fee:this.pay_money}).then((res)=>{
              if(res.data.res){
